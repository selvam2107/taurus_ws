...
def end():
    ...
    global odo1, odo2,m,alarm1,alarm2,tt,doc_count
    print(alarm1)
    dia=0.25
    gear_ratio=40
    global col
    ...
    print(m)
    for i in col.find({},{"wheel1 distance in m":1, "wheel2 distance in m":1,"termination count":1,"total_time":1,"docking_count":1}):
        s=i
        print(i)
    g=s.get("wheel1 distance in m",0)+wheel1
    n=s.get("wheel2 distance in m",0)+wheel2
    dc=s.get("docking_count",0)+int(doc_count)
    ...
    print(count)
    col.update_one({"_id":m},{"$set": { "wheel1 distance in m": g,"wheel2 distance in m":n,"total_distance in m":total_distace,"termination count":count,"total_time":tm,"docking_count":dc}})
...
if __name__ == '__main__':
    ...
    myclient = pymongo.MongoClient("mongodb://192.168.5.7:27017")
...
'''
    ...
    c=rospy.Subscriber("d_count",Int64,d_count,queue_size=2)
    print("wibh")
