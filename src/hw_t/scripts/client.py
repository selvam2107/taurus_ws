...
class Task:
    ...
    def path_callback(self, msg):
        # Get the path from the /path topic and slice it into chunks
        self.path = [(pose.pose.position.x, pose.pose.position.y) for pose in msg.poses]
        self.path.reverse()
        # For example, here we slice the path into chunks
    ...
    def move_to_waypoint(self, goal_x, goal_y):
        ...
        while not rospy.is_shutdown():
            ...
            # Determine if reverse motion is required
            move_forward = fabs(yaw_error) < pi / 2  # Forward if yaw error is less than 90 degrees
            ...
            twist = Twist()
            twist.linear.x = linear_velocity if move_forward else -linear_velocity  # Reverse motion
            twist.angular.z = angular_velocity
            self.cmd_vel.publish(twist)
            rate.sleep()
    ...
    def shutdown(self):
        ...
        self.cmd_vel.publish(Twist())  # Stop robot
        rospy.sleep(1)
...
if __name__ == '__main__':
    task = Task()
    rospy.sleep(1)
    task.move()  # Start moving robot along the path
